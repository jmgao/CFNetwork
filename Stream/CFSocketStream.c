	...
	if (!response) {
		...
		if (!left) {
			...
			do {
				...
				urlstr = CFStringCreateWithFormat(alloc, NULL, _kCFStreamCONNECTURLFormat, name, (int)p & 0x0000FFFF);
	...
	if (!result && !peerid) {
		...
		if (value) {
			...
			if (host && port) {
				...
				peer = CFStringCreateWithFormat(alloc, NULL, _kCFStreamCONNECTURLFormat, host, (int)p & 0x0000FFFF);
